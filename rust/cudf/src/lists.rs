...
pub trait ListExt {
    ...
    fn list_index_of_column(
        &self,
        search_keys: &ColumnView<'_>,
        find_first: bool,
    ) -> Result<Column>;
    /// Gather elements from each list row using a gather map list column.
    fn list_segmented_gather(
        &self,
        gather_map: &ColumnView<'_>,
        nullify_oob: bool,
    ) -> Result<Column>;
...
pub fn list_sequences_with_step(
    starts: &ColumnView<'_>,
    steps: &ColumnView<'_>,
    sizes: &ColumnView<'_>,
) -> Result<Column> {
    let c = cudf_sys::ffi::lists_sequences_with_step(
        starts.0,
        steps.0,
        sizes.0,
...
impl ListExt for ColumnView<'_> {
    ...
    fn list_extract_element(&self, index: i32) -> Result<Column> {
        let c =
            cudf_sys::ffi::lists_extract_element(self.0, index, Stream::default_stream().as_raw())?;
        Ok(Column(c))
    }
    fn list_sort(&self, ascending: bool, nulls_last: bool) -> Result<Column> {
        let c = cudf_sys::ffi::lists_sort(
            self.0,
            ascending,
            nulls_last,
            Stream::default_stream().as_raw(),
    ...
    fn list_concatenate_elements(&self) -> Result<Column> {
        let c =
            cudf_sys::ffi::lists_concatenate_elements(self.0, Stream::default_stream().as_raw())?;
        Ok(Column(c))
    }
    fn list_contains_scalar(&self, search_key: &Scalar) -> Result<Column> {
        let ffi = crate::scalar::scalar_to_ffi(search_key);
        let c =
            cudf_sys::ffi::lists_contains_scalar(self.0, &ffi, Stream::default_stream().as_raw())?;
        Ok(Column(c))
    }
    fn list_contains_column(&self, search_keys: &ColumnView<'_>) -> Result<Column> {
        let c = cudf_sys::ffi::lists_contains_column(
            self.0,
            search_keys.0,
            Stream::default_stream().as_raw(),
        )?;
    ...
    fn list_index_of_scalar(&self, search_key: &Scalar, find_first: bool) -> Result<Column> {
        let ffi = crate::scalar::scalar_to_ffi(search_key);
        let c = cudf_sys::ffi::lists_index_of_scalar(
            self.0,
            &ffi,
            find_first,
            Stream::default_stream().as_raw(),
    ...
    fn list_index_of_column(
        ...
        let c = cudf_sys::ffi::lists_index_of_column(
            self.0,
            search_keys.0,
            find_first,
            Stream::default_stream().as_raw(),
    ...
    fn list_segmented_gather(
        ...
        let c = cudf_sys::ffi::lists_segmented_gather(
            self.0,
            gather_map.0,
            nullify_oob,
            Stream::default_stream().as_raw(),
    ...
    fn list_format(&self, na_rep: &str) -> Result<Column> {
        let c = cudf_sys::ffi::strings_format_list_column(
            self.0,
            na_rep,
            Stream::default_stream().as_raw(),
        )?;
    ...
    fn list_extract_element_column(&self, indices: &ColumnView<'_>) -> Result<Column> {
        let c = cudf_sys::ffi::lists_extract_element_column(
            self.0,
            indices.0,
            Stream::default_stream().as_raw(),
        )?;
    ...
    fn list_stable_sort(&self, ascending: bool, nulls_last: bool) -> Result<Column> {
        let c = cudf_sys::ffi::lists_stable_sort(
            self.0,
            ascending,
            nulls_last,
            Stream::default_stream().as_raw(),
    ...
    fn list_apply_boolean_mask(&self, boolean_mask: &ColumnView<'_>) -> Result<Column> {
        let c = cudf_sys::ffi::lists_apply_boolean_mask(
            self.0,
            boolean_mask.0,
            Stream::default_stream().as_raw(),
        )?;
...
pub fn lists_intersect_distinct(lhs: &ColumnView<'_>, rhs: &ColumnView<'_>) -> Result<Column> {
    let c =
        cudf_sys::ffi::lists_intersect_distinct(lhs.0, rhs.0, Stream::default_stream().as_raw())?;
    Ok(Column(c))
}
...
pub fn lists_difference_distinct(lhs: &ColumnView<'_>, rhs: &ColumnView<'_>) -> Result<Column> {
    let c =
        cudf_sys::ffi::lists_difference_distinct(lhs.0, rhs.0, Stream::default_stream().as_raw())?;
    Ok(Column(c))
}
