...
pub fn inner_join(left: &Table, right: &Table, left_on: &[i32], right_on: &[i32]) -> Result<Table> {
    let t = cudf_sys::ffi::inner_join(&left.0, &right.0, left_on, right_on, ds())?;
    Ok(Table(t))
}
...
pub fn left_join(left: &Table, right: &Table, left_on: &[i32], right_on: &[i32]) -> Result<Table> {
    let t = cudf_sys::ffi::left_join(&left.0, &right.0, left_on, right_on, ds())?;
    Ok(Table(t))
}
...
pub fn full_join(left: &Table, right: &Table, left_on: &[i32], right_on: &[i32]) -> Result<Table> {
    let t = cudf_sys::ffi::full_join(&left.0, &right.0, left_on, right_on, ds())?;
    Ok(Table(t))
}
...
pub fn left_semi_join(
    ...
    let t = cudf_sys::ffi::left_semi_join(&left.0, &right.0, left_on, right_on, ds())?;
    Ok(Table(t))
}
...
pub fn left_anti_join(
    ...
    let t = cudf_sys::ffi::left_anti_join(&left.0, &right.0, left_on, right_on, ds())?;
    Ok(Table(t))
}
...
mod tests {
    ...
    fn inner_join_basic() {
        ...
        let result = inner_join(&left, &right, &[0i32], &[0i32]).unwrap();
    ...
    fn left_join_basic() {
        ...
        let result = left_join(&left, &right, &[0i32], &[0i32]).unwrap();
    ...
    fn full_join_basic() {
        ...
        let result = full_join(&left, &right, &[0i32], &[0i32]).unwrap();
        assert_eq!(result.columns_len(), 4);
    ...
    fn left_semi_join_basic() {
        ...
        let result = left_semi_join(&left, &right, &[0i32], &[0i32]).unwrap();
    ...
    fn left_anti_join_basic() {
        ...
        let result = left_anti_join(&left, &right, &[0i32], &[0i32]).unwrap();
        assert_eq!(result.columns_len(), 2); // only left columns
